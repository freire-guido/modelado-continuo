...
# ╔═╡ 5588e12a-d1e0-45cf-bd8d-a8a1bd751a32
begin
	x0 = [100, 5, 0]
	t = 1:42
	tspan = (1, 42)
	prob_SIR = ODEProblem(SIR, x0, tspan, [1., 1., 100])
end

# ╔═╡ 9c96b549-994a-4be0-b422-4e406691c6b4
data_infec = df[!, 7]

# ╔═╡ 48a68258-45e0-41b7-887e-b89832ac9c8d
function loss_SIR(sol)
	sol_infec = sol.(t,idxs=2)
	return sum((sol_infec - data_infec).^2)
end

# ╔═╡ 63b2d9e3-ec16-4438-8332-323041d99902
func_SIR = 
	build_loss_objective(prob_SIR,Tsit5(),loss_SIR,Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)

# ╔═╡ eb778f05-be6d-46a6-8865-c2c3216be369
optprob_SIR = OptimizationProblem(func_SIR, [1., 1., 100])

# ╔═╡ a950844f-5c24-4fdd-b904-7dde7f698e7f
begin
...
# ╔═╡ 2f6f754a-cf5d-4a7f-9f83-fff5a461bc61
begin
	plot(sol_SIR₂)
	scatter!(t, data_infec)
end

# ╔═╡ d8626a3f-6bf7-4662-bc7d-c56ac6715753
md"""# Pred-presa"""

# ╔═╡ 56916c8a-f00b-4d3a-8072-1c2e8c5f8cce
df_pp = CSV.read("lynxhare.csv", DataFrame)

# ╔═╡ e20fa4ff-b799-4c00-a207-f00609e7fe3e
function pred_presa(x, p, t)
	u, v = x
	α, β, γ, δ = p
	du = -α*u+γ*u*v
	dv = β*v - δ*u*v
...
# ╔═╡ 46390d36-5a33-4c26-b51f-eb053823ae7c
begin
	x0_pp = [30, 20]
	t_pp = 1845:1935
	tspan_pp = (1845, 1935)
	p0_pp = [0.25, 1, 0.01, 0.01]
	prob_pp = ODEProblem(pred_presa, x0_pp, tspan_pp, p0_pp)
end

# ╔═╡ c938f335-5a5b-49de-9201-e5917712fe7b
plot(solve(prob_pp))

# ╔═╡ f7e0ae42-bd0b-4ab5-b49b-6d10016291a8
[df_pp[!, 3] df_pp[!, 2]]

# ╔═╡ a4458077-4d5f-431d-925d-0f3ca705f907
begin
	func_pp = 
		build_loss_objective(prob_pp,AutoTsit5(Rosenbrock23()),L2Loss(t_pp, [df_pp[!, 3] df_pp[!, 2]]),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)
	optprob_pp = OptimizationProblem(func_pp, 0.1*rand(4), lb = zeros(4), ub= [2.,2.,2.,2.] )
end

# ╔═╡ 625720a4-1718-4ca3-a038-867200c19587
p_pp = solve(optprob_pp, SAMIN(rt=0.5), maxiters = 20000)

# ╔═╡ 6206eea1-e037-4469-82fc-3f32995ff8db
prob_pp₂ = remake(prob_pp, p=p_pp)

# ╔═╡ f0706f97-94de-4e53-a7d7-0267ce913a01
begin
	plot(solve(prob_pp₂))
	scatter!(t_pp, [df_pp[!, 3], df_pp[!, 2]])
end

# ╔═╡ 2f8ea336-1455-4ff0-bc4c-3f620095c506


# ╔═╡ f41e2914-a764-4499-bbc5-2e0015151109
