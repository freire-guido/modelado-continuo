...
using DataFrames, CSV, Plots, DifferentialEquations, Optimization, DiffEqParamEstim,OptimizationOptimJL,Dates, PlutoUI, Random

# ╔═╡ 915128de-2cf2-489b-8502-342f99e4a467
using GLMakie: streamplot, Point2

# ╔═╡ f8693f9f-e8fe-4652-8540-d2c0d3124e6c
theme(:dracula)

# ╔═╡ b7aa13c5-c7ac-499a-a499-3f591140d4b9
md"""
Dejamos parte del enunciado en la entrega para que se entienda nuestro proceso de implementación.
Los encabezados en _cursiva_ son nuestras conclusiones.
"""
...
begin
	...
	sel = argentina[58:7:1336, [1,6,8]]
end

# ╔═╡ 3a58ceca-f93f-488d-8c28-8a663ab21f75
begin
	weekly_cases = prepend!(diff(sel[!,2]),0)  / 45_500_000
	weekly_death = prepend!(diff(sel[!,3]),0)  / 45_500_000
end

# ╔═╡ 1a3f34c6-9ad0-4420-a30e-a529ac7d0e0b
begin
	tf1 = 42 # ola 1
	tf2 = 52 # ola 2
	tfe = 88 # fin
...
# ╔═╡ 6e4a0603-d3c0-4630-b9a3-484e782df123
begin
	plot(weekly_cases)
	vline!([tf1], label = "Ola 1")
	vline!([tf2], label = "Ola 2")
	vline!([tfe], label = "Fin")
end
...
Dado σ, bajar β tiene el efecto contrario: La ola es más tardía y plana, esto es exactamente a lo que en pandemia llamabamos "aplanar la ola". Fijar β y aumentar σ impacta la velocidad de "remoción" de los infectados. Esto también aplana la ola, que es justamente lo que hizo al COVID tan grave; tenia un σ bajísimo.
"""
...
Los valores iniciales del ODEProblem son opcionales. Si no los recibe, hay que pasarle un ```generator```, que es el prob_generator de la función objetivo.
"""
...
function optimizar_params(f, p₀, tf, loss, solver, x₀ = nothing; lb = nothing, ub = nothing, generator = DiffEqParamEstim.STANDARD_PROB_GENERATOR, maxiters = 1000)
	if x₀ == nothing
...
Primero usamos ```optimizar_params``` sin generator y un x₀ fijo, como pide el enunciado.
"""
...
begin
	sol_x0 = solve(prob_x0)
	plot(sol_x0, xlim = (0, tfe), ylim = (0, 0.005), label = ["S" "I" "R"])
	plot!(p_x0[2]*sol_x0.(1:tf1, idxs = 1).*sol_x0.(1:tf1, idxs = 2), label = "βIS")
	scatter!(weekly_cases, label = "WHO")
end

# ╔═╡ 169e4aa4-16e6-4807-b605-cede51ad26d2
md"""
El ajuste es bastante malo, porque el "ε" esta fijado a ojo. Para optimizarlo usamos le pasamos el generator y un parámetro más a la función.
"""

# ╔═╡ 1e84ab86-cd37-4af3-b6e3-ee6f336215c9
md"""
## _Optimizando x₀_
Pasamos un remake como generator para que reste el primer parámetro a los valores iniciales, como lo haría un ε en el modelo.
"""
...
begin
	...
	plot(sol_SIR, xlim = (0, tfe), ylim = (0, 0.005), label = ["S" "I" "R"])
...
Ahora los ajustes son mejores, graficamos S, I, R para cada modelo, así como el aporte de nuevos infectados βIS, que interpola los datos bastante bien. Con SAMIN llegamos a minimizar los costos al orden 10⁻⁷, y SEIR es mejor que SIR (que era de esperarse) aunque marginalmente. Solamente estamos ajustando una ola.
"""

# ╔═╡ 7e2c93b1-fe46-4c46-b546-3c04b9028795
md"""
# _Interpretación de los parámetros_
"""

# ╔═╡ 007b16df-a42e-46be-bb2b-522d9ebca7f8
p_SIR

# ╔═╡ 001b0388-9818-4f44-9f0e-505eaef410e8
p_SEIR

# ╔═╡ f863edc5-c2b6-4713-b1dc-b7d8736a1742
md"""
* β: en SIR ~ 11, esto se interpreta como un tiempo medio entre contagio de 1/11 ~ 0.09 días entre contagios. En SEIR este tiempo es incluso menor, 1/36 ~ 0.03.
* γ: en SEIR ~ 35, casi lo mismo que la tasa de contagios β.
* σ: en SIR ~ 11, que es muy parecido a la tasa de contagio. En SEIR ~ 15.
"""
...
md"""
Si bien parece ajustar bien, comparemos el costo de SEIRS con SEIR hasta la segunda ola"""
...
md"""
El costo de SEIRS es un orden de magnitud mejor para los datos hasta la segunda ola
"""
...
SEIRS no ajusta las dos olas porque asume que todos los parámetros son constantes, que sobresimplifica el problema. El principal cambio en los parámetros se da en una reducción del β con el correr de la pandemia.

Implementamos el cambio más simple, que es modificar el β por un β₂ en un dia fijo (la segunda ola). Vemos que funciona mejor:
"""

# ╔═╡ faa9e056-2c53-4cd3-a24e-15dd593beaa3
function SEIRS_C(x, p, t)
	S, E, I, R = x
	β1, β2, σ, γ, δ = p
	β = β1
	if (t >= tf1)
...
md"""
Hacemos un streamplot del dominio S, I para un modelo SIR (ya que R = 1 - S - I)
"""

# ╔═╡ 9d3d1bc6-29da-411c-844a-738441f88de9
begin
	f(S, I) = Point2(-p_SIR[2]*S*I, p_SIR[2]*S*I - p_SIR[3]*I)
	streamplot(f, 0:3, 0:3)
end

# ╔═╡ ef894257-7a52-407f-bda7-a99eb1d55e12
md"""
Tambien graficamos la trayectoria de S, I, R para SEIRS. En un dominio 4d estaría contenida en un hipercubo, ya que S + E + I + R (norma-1) es constante.
"""

# ╔═╡ 8de17596-8c1b-4cb1-b008-a2114bfe24aa
begin
	ts = 1:0.5:tfe
	@gif for t in ts
		p = plot(sol_SEIRS(ts, idxs = 1), sol_SEIRS(ts, idxs = 3), sol_SEIRS(ts, idxs = 4), xlabel = "S", ylabel = "I", zlabel = "R", ticks = false)
		scatter!([sol_SEIRS(t)[1]], [sol_SEIRS(t)[3]], [sol_SEIRS(t)[4]])
		plot!(p[1], camera = (t, tfe-t))
	end
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "bb05d1c94011b8507e4bfb372ff85958988e5732"
...
git-tree-sha1 = "38f68ad935bba79836daa66b92285d8deb708c29"
...
git-tree-sha1 = "deb3a7f3dd520fb90e38c4d77fc1b41ee4e68f68"
