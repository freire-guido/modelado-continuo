...
# ╔═╡ dc3ba192-57b4-11ee-1cd1-13685989a5bf
using Plots,DifferentialEquations

# ╔═╡ 15969905-5225-40bd-ba47-128c1a9e357d
...
# ╔═╡ 5ef38fd6-49af-45d0-b44a-cf86030f46cd
begin
	prob_cañon = ODEProblem(caida, [55.8, 0], (0, 10), [0.0058, 16, 0])
	prob_mosquete = ODEProblem(caida, [55.8, 0], (0, 10), [3.74e-5, 0.0082, 0])
end

# ╔═╡ 9b89f66d-152b-4a9b-ba4f-67ad0714323f
begin
	plot(solve(prob_cañon))
	plot!(solve(prob_mosquete))
end

# ╔═╡ afc2f5ec-f41b-4b83-a47f-1d53c1b98404
md"""
Como traspasa la tierra, callbackeamos
"""

# ╔═╡ 0e0435d2-4ff3-48a9-814f-53dd2fb58cf1
function cond_piso(u, t, integrator)
	min(u[1])
end

# ╔═╡ b95f2a12-f907-4704-a13f-d076b4a2ca37
callback_piso = ContinuousCallback(cond_piso, terminate!)

# ╔═╡ 3f5da78a-9734-4b83-abbf-91a201a85760
begin
	plot(solve(prob_cañon, callback = callback_piso))
	plot!(solve(prob_mosquete, callback = callback_piso))
end

# ╔═╡ 0545243b-fe56-43ca-907e-a1fb16d7017b
md"""
Hagamos que reboten
"""

# ╔═╡ 8a978289-38b9-4cd2-9c19-fd065aefdb9e
prob_rebote = ODEProblem(caida, [55.8, 0], (0, 40), [0.5, 16, 0.92])

# ╔═╡ 25796b2c-c851-462e-902e-f63b9d09c647
accion_rebote!(integrator) = integrator.u[2] = -integrator.p[3].*integrator.u[2]

# ╔═╡ eb31fc05-19bc-417d-a1c5-d9840bd1c2cb
callback_rebote = ContinuousCallback(cond_piso, accion_rebote!)

# ╔═╡ 324ed514-aef5-4225-a18c-2966e1f7524e
plot(solve(prob_rebote, callback = callback_rebote))

# ╔═╡ 20bd8963-c8b4-434c-9cdb-12453293f949
prob_muchas = ODEProblem(caida, [[100, 0] [50, 0]], (0, 100), [[0.0058, 16, 0.99] [3.74e-5, 0.0088, 0]])

# ╔═╡ e855eea2-12bf-4e48-88a4-dda0b7cb1c45
coso(integrator) = return

# ╔═╡ 4feae3e5-0afd-4ff8-b81f-d2771085f244
callback_muchas = ContinuousCallback(cond_piso, accion_rebote!)

# ╔═╡ 01b63f86-218c-48c1-8135-21002586a0b9
plot(solve(prob_muchas), callback = callback_muchas)
